...
def grid_gen(start_coord, length, nx, ny, nz):
   ...
   return grid[:,[2,1,0]]
def main():
   mykf_frag1 = KFFile(input('Enter KFFile of fragment 1: '))
   mykf_frag2 = KFFile(input('Enter KFFile of fragment 2: '))
   mol = Molecule(input('Enter xyz file of the complex: '))
   sym = input('Enter symmetry of the fragment orbitals: ')
   MO1 = input('Enter orbital of fragment 1: ')
   try:
       iMO1 = int(MO1)
   except:
       print('Invalid input')
   MO2 = input('Enter orbital of fragment 2: ')
   try:
       iMO2 = int(MO2)
   except:
       print('Invalid input')
   density_frag1      = mykf_frag1['SCF_'+sym+'%'+MO1]
   density_frag2      = mykf_frag2['SCF_'+sym+'%'+MO2]
