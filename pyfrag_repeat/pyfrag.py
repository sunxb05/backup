...
pp.add("Distance", specs, r'print\s*_?distance\s*=?\s*(\d\s*\d\s*\d.*)', dim = 2, unit='Angstrom')
...
if pp.Distance.args is not None:
    for i in range(len(pp.Distance.args)):
        distances = []
        if len(pp.Distance.args[i])>3:
            diffa = float(pp.Distance.args[i][-1])
        else:
            diffa = 0
        for xyz in matrix:
            distances.append(distance_xyz(xyz, pp.Distance.args[i][0], pp.Distance.args[i][1], pp.Distance.args[i][2]) - diffa)
        pp.Distance.args[i] = distances
...
for i in calc_points:
        ...
        if matrix_list_frag1:
                ...
                new_name_frag1_t21_1 = shelljoin(tape21_save_dir,new_name_frag1+'.t21')
#                new_adf(adfscript, frag_dir, new_name_frag1, new_name_frag1_t21, matrix_list_frag1[i], extra_frag1, title='frag1')
#                print(new_name_frag1_t21_1)
                shutil.copy(new_name_frag1_t21_1, '.')
                if pp.StrainFrag1.args is not None:
                    frag1_t21 = kffile.kffile(new_name_frag1_t21_fa)
        ...
        if matrix_list_frag2:
                ...
                tape21_save_dir = shelljoin(frag_dir, 'TAPE21s')
                new_name_frag2_t21_1 = shelljoin(tape21_save_dir,new_name_frag2+'.t21')
#                new_adf(adfscript, frag_dir, new_name_frag2, new_name_frag2_t21, matrix_list_frag2[i], extra_frag2, title='frag2')
                shutil.copy(new_name_frag2_t21_1, '.')
                if pp.StrainFrag2.args is not None:
                    frag2_t21 = kffile.kffile(new_name_frag2_t21_fa)
...
#                strain_frag2_new = False
        ...
        new_name_fa_t21_fa = os.path.basename(new_name_fa+'.t21')
        tape21_save_dir = shelljoin(frag_dir, 'TAPE21s')
        new_name_fa_t21_1 = shelljoin(tape21_save_dir,new_name_fa+'.t21')
        shutil.copy(new_name_fa_t21_1, '.')                
...
#        if matrix_list_frag2: os.remove(new_name_frag2_t21_fa)        
        ...
        current_fa_t21 = kffile.kffile(new_name_fa_t21_fa)
...
#                        energy_file.write(string.ljust(pp[key].unit, pp[key].pwidth))
        ...
        if pp.Distance.args is not None:
            pp.Distance.cv = []
            for bond in range(len(pp.Distance.args)):
                pp.Distance.cv.append(pp.Distance.args[bond][i])
