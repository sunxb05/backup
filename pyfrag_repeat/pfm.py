...
#    nocore_mo_count = t21.read(irrep, 'nmo_A')[0]
    core_orb_irrep = []
    irreps = read_irreps_t21(t21)
    number_core_irrep = t21.read('Symmetry', 'ncbs')
    for i, n in zip(irreps, number_core_irrep):
        if i != irrep: continue
        core_orb_irrep.append([i,n])
#    return [core_orb_irrep[0]][-1]   
    return core_orb_irrep[0][-1]
#    return 0   
...
def find_holu_t21_pop(t21, frag_nr, holu, frag_nrs = False):
    ...
    for i, nr in zip(irreps, nr_of_SFOs_per_irrep):
        for orb in range(1, nr+1):
            irreps_fa.append(i)
    number_core = t21.read('Symmetry', 'ncbs')
    orb_number = []
    orb_numbers = []
    a = 0
    for m, n in zip(nr_of_SFOs_per_irrep, number_core):
        b = 0
        b = m + n
        a += b
        orb_number.append(a)
    for x, y in zip(nr_of_SFOs_per_irrep, orb_number):
        z = range(1, y+2)
        orb_numbers.extend(z[-x-1 : -1])
    frag_orbs = []
    for e, n, o, i, f, fn, onf, irf in zip(energies, orb_numbers, occs, irreps_frags, frag_orb_nrs, fragment_nr, orb_nrs_fa, irreps_fa):
        if re.search('HOMO', holu) and o == 0.0: continue
        if re.search('LUMO', holu) and o != 0.0: continue
        if fn == int(frag_nr): frag_orbs.append([e, n, o, i, f, fn, onf, irf])
    frag_orbs.sort()
    ...
    if re.search('HOMO', holu):
        ...
        holu_orb_pop = frag_orbs[-1+homo_index][-7]
    if re.search('LUMO', holu):
        ...
        holu_orb_pop = frag_orbs[homo_index - 1][-7]
...
#        sys.stderr.write('\nWrong irreps.\n')
#        return None
    orb_nr = find_holu_t21_pop(t21, frag_nr, orb_nr)
#    print (orb_nr)
...
# distance between metal (atom1) and bond to be activated (atom2_atom3)
def distance_xyz(xyz,atom1, atom2, atom3):

    try:
        angle = [int(atom1), int(atom2), int(atom3)]
    except:
        sys.stderr.write("values for printing distance are not integers!")
        return None

    vec1 = [xyz[angle[0]-1][0] - xyz[angle[1]-1][0], xyz[angle[0]-1][1] - xyz[angle[1]-1][1], xyz[angle[0]-1][2] - xyz[angle[1]-1][2]]
    vec2 = [xyz[angle[2]-1][0] - xyz[angle[1]-1][0], xyz[angle[2]-1][1] - xyz[angle[1]-1][1], xyz[angle[2]-1][2] - xyz[angle[1]-1][2]]
    n_vec2 = vector_normalize(vec2)
    distancevalue = vector_innerproduct(vec1, n_vec2)
