...
for i in loop:
    spec = []
    if re.compile(r'Standard orientation:', re.IGNORECASE).search(gsscript):
        if re.compile(r'Standard orientation:', re.IGNORECASE).search(gsscript):
            spec = re.compile(r'Standard orientation:'+r'.*?'+ r'Rotational constants', re.IGNORECASE|re.DOTALL).findall(gsscript)[i]
            spec = re.compile(r'Rotational constants'+r'.*', re.IGNORECASE).sub('', spec)
            spec = re.compile(r'Standard orientation:'+r'.*', re.IGNORECASE).sub('', spec)
            newspec = []
