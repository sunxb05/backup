...
def grid_gen(start_coord, length, nx, ny, nz):
   ...
   return grid[:,[2,1,0]]
def main():
   mykf_frag          = KFFile('test')
   density_frag1      = mykf_frag['SFO_A%3']
   density_frag2      = mykf_frag['SFO_A%64']
   overlap_density    = [a * b for a,b in zip(density_frag1, density_frag2)]
   nx                 = mykf_frag["Grid%nr of points x"]
   ny                 = mykf_frag["Grid%nr of points y"]
   nz                 = mykf_frag["Grid%nr of points z"]
   start_coord        = [i*unit for i in mykf_frag['Grid%Start_point']]
   atom_coord         = mykf_frag["Geometry%xyznuc"]
   atom_coord         = [atom_coord[i:i+3] for i in range(0, len(atom_coord), 3)]
   coordinates        = (np.array(atom_coord))*(unit) # KFFile changes the units to Bohr
   natoms             = len(coordinates)
   x_vector           = mykf_frag['Grid%x-vector']
   length             = x_vector[0]
   weight             = length**3
   at_num             = mykf_frag["Geometry%qtch"]
   labels             = mykf_frag["Geometry%labels"]
